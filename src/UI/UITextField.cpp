...
UITextField::UITextField(SDL_Renderer* renderer, SDL_Surface* charset, double x, double y, int width, int height) : UIElement(renderer, x, y, width, height)
{
    isEditable = true;
    mouseX = mouseY = 0;
    length = 0;
    capacity = width / (height - (TXTFIELD_TEXT_OFFSET * 2));
    textBuffer = new char[capacity + 1];
    textBuffer[0] = '\0';

    backgroundComponent = new UIRectangle(renderer, x, y, width, height, 255, 255, 255);
    textComponent = new UIText(renderer, charset, x + TXTFIELD_TEXT_OFFSET, y + TXTFIELD_TEXT_OFFSET, width - (TXTFIELD_TEXT_OFFSET * 2), height - (TXTFIELD_TEXT_OFFSET * 2), "");
    textComponent->setColor(0, 0, 0);
}

void UITextField::update()
{
    // printf("Text field active: %d; Length: %d / %d; Text: %s\n", isEditable, length, capacity, textBuffer);
    // printf("Kanapka\n");
...
void UITextField::appendText(const char* textToAppend)
{
    ...
    if(length + textToAppendLength > capacity) return;

    memcpy(textBuffer + length, textToAppend, textToAppendLength);
